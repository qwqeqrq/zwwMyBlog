...
public class DateUtil {
    ...
    /**
     * 根据用户传入的时间字符串，传入的日期格式，判断时间字符串是否满足日期格式。
     *
     * @param date1,sformat
     * @return
     */
    public static boolean dateFormatTest(Date date1, String sformat) {
        boolean flag = true;
        try {
            SimpleDateFormat formatter = new SimpleDateFormat(sformat);
...
//    sdate = VeStr.Replace(sdate, "/", "-");
     ...
     * 输入格式 字符串 yyyy-MM-dd
     */
    public static int getDaysOfYear(String dateStr) {
        int dayOfYear = 0;
        //如果不满足格式 返回0 表示错误
        ...
        try {
            String[] dateStr1 = dateStr.split("-");
            LocalDate date = LocalDate.of(Integer.parseInt(dateStr1[0]), Integer.parseInt(dateStr1[1]), Integer.parseInt(dateStr1[2]));
    ...
    /**
     * 转换时间格式 yyyy-MM-dd HH:mm:ss 转换为yyyy-MM-dd
     *
     * @param date
     * @return
     */
    public static Date formatLongDateToShortDate(Date date) {
        return strToDate(formatter.format(date));
    }
     ...
     * 返回输入时间最近的整点时间
     */
    public static String getNearestDate(String s) {
        //取出分钟
        String minuteStr = s.substring(14, 16);
        if ("".equals(minuteStr))
            return "";
        Integer minute = Integer.valueOf(minuteStr);
        Integer minutes = 60 - Integer.valueOf(minuteStr);
        if (minute >= 30) {
            String date = getPreTime(s, minutes.toString());
            return new StringBuilder(date).replace(17, 19, "00").toString();
        } else {
            String date = getPreTime(s, minute.toString());
            return new StringBuilder(date).replace(17, 19, "00").toString();
        }
    }
}